...
function renderModelsModal(brand) {
    ...
    // Получаем модели из базы данных - показываем ВСЕ модели марки без фильтрации по типу
    let availableModels = [];
    
    // Используем новый формат базы данных, если доступен
    if (typeof carDatabaseArray !== 'undefined') {
        // Показываем все модели выбранной марки без фильтрации по типу
        const modelsMap = new Map();
        carDatabaseArray.forEach(car => {
            if (car.brand === brand) {
                // Если модель уже есть, но с другим типом, добавляем её тоже
                if (!modelsMap.has(car.model)) {
                    modelsMap.set(car.model, car);
                } else {
                    // Если модель уже есть, но текущая имеет более подходящий тип, обновляем
                    const existing = modelsMap.get(car.model);
                    // Приоритет: если выбран тип и текущая модель соответствует типу, используем её
                    if (selectedType && typeCategoryMapping[selectedType]) {
                        const allowedTypes = typeCategoryMapping[selectedType];
                        if (allowedTypes.includes(car.type) && !allowedTypes.includes(existing.type)) {
                            modelsMap.set(car.model, car);
                        }
                    }
                }
        ...
        availableModels = Array.from(modelsMap.values()).map(car => ({
            model: car.model,
            type: car.type,
            body: car.body
        }));
